...
[ "$#" = 4 ] || die 'usage: install-root.sh BOOT_ID ROOTFS_URL SHA256 ERASE-LIUQIN-USERDATA'
[ "$4" = ERASE-LIUQIN-USERDATA ] || die 'explicit data-erasure acknowledgement required'
...
# Download and validate before any destructive operation.
/bin/busybox wget -O /run/rootfs.tar.gz "$2"
printf '%s  /run/rootfs.tar.gz\n' "$3" | /bin/busybox sha256sum -c -
/usr/bin/tar -tzf /run/rootfs.tar.gz >/dev/null
...
mkdir -p /run/persist /mnt/install
...
/usr/bin/tar -xzf /run/rootfs.tar.gz -C /mnt/install/native-root \
